...
void testPosture() {
  for (byte s = 0; s < 12; s++)
    myservo[s].write(zero[s] + shift[s]);  // tell servo to go to position in variable 'pos'
  delay(2000);
  for (byte s = 0; s < 12; s++)
    myservo[s].write(zero[s] + shift[s] + stand[s]);  // tell servo to go to position in variable 'pos'
  delay(2000);
  for (byte s = 0; s < 12; s++)
    myservo[s].write(zero[s] + shift[s] + rest[s]);  // tell servo to go to position in variable 'pos'
  delay(2000);
  // for (byte s = 0; s < 12; s++)
  //   myservo[s].write(stand[s]);  // tell servo to go to position in variable 'pos'
  // delay(1000);
}
...
void testGait() {
  for (pos = -amplitude; pos <= +amplitude; pos += 1) {
    for (byte s = 0; s < 12; s++)
      myservo[s].write(pos * (s % 4 / 2 ? 1 : -1) * (s > 7 ? 1 : 1)
                       + zero[s] + shift[s] + stand[s]);  // tell servo to go to position in variable 'pos'
