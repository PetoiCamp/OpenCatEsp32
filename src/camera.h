...
#endif
  {
    ...
    if (abs(xDiff) > 1 || abs(yDiff) > 1) {
      if (abs(currentX) < 45) {
        ...
        for (int i = 0; i < DOF; i++) {
          allParameter[i] = min(125, max(-125, base[i] + (feedBackArray[i][0] ? currentX * 10 / feedBackArray[i][0] : 0) + (feedBackArray[i][1] ? currentY * 10 / feedBackArray[i][1] : 0)));
        }
        printList(allParameter);
        cmdLen = DOF;
        token = T_LISTED_BIN;
        for (byte i = 0; i < cmdLen; i++)
          newCmd[i] = (int8_t)min(max(allParameter[i], int8_t(-125)), int8_t(125));
        newCmd[cmdLen] = '~';
        // newCmdIdx = 6;
        tQueue->addTask(T_LISTED_BIN, newCmd);
        //      printList(newCmd);}
      }
#ifdef ROTATE
      else {
