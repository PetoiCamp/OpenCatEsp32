...
template <typename T> void transform( T * target, byte angleDataRatio = 1, float speedRatio = 1, byte offset = 0,int period = 0,int runDelay=8) {
    ...
    Serial.print(" target_max_abs_svel: ");Serial.print(target_max_abs_svel);
    Serial.print("\n curr_max_abs_svel: ");Serial.print(curr_max_abs_svel);
    ...
    delete [] cAng_cp;
      ...
      for (int i = offset; i < DOF; i++){
        ...
        svel[i-offset] =min(max(currentAng[i]-previousAng[i],-svel_clip),svel_clip);
      }
      int steps = speedRatio > 0 ?  max (int(round(maxDiff / 1.0/*degreeStep*/ / speedRatio)),1) : 1 ; //default speed is 1 degree per step
